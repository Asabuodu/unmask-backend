...
// import { Game, Statement, Guess } from '../types';
...
//   async handlePlayerJoin(sessionCode: string, userId: string) {
//     return this.sessionsService.addPlayer(sessionCode, userId);
//   }

//   async handlePlayerLeave(sessionCode: string, userId: string) {
...
//   /* Statement & Vote */
//   async createStatement(owner: string, text: string, type: 'truth' | 'lie') {
//     return this.statementsService.create(text, type, owner);
//   }

//   async castVote(voter: string, statementId: string, guess: 'truth' | 'lie') {
//     return this.votesService.castVote(voter, statementId, guess);
//   }

//   async getVoteCounts(statementId: string) {
//     const votes = await this.votesService.findByStatement(statementId);
//     const counts = { truth: 0, lie: 0 };
//     votes.forEach((v: any) => {
...
//    * - Determine actual type for the statement (type stored on statement)
//    * - Count correct guesses
//    * - If >=2 guessed correctly -> owner eliminated
//    * - If 0 guessed correctly -> owner chooses a player to eliminate (we return a flag so host/client can prompt owner)
//    * - Otherwise no elimination.
//    */
//   async processRound(sessionCode: string, statementId: string) {
//     const session = await this.sessionsService.getSession(sessionCode);
//     if (!session) throw new BadRequestException('Session not found');
...
//     const votes = await this.votesService.findByStatement(statementId);
//     const actualType = statement.type; // 'truth' | 'lie'
//     const correctVotes = votes.filter((v: any) => v.guess === actualType);

//     // result object to return to clients
//     const result: any = {
//       statementId,
...
//       // owner must eliminate someone — return flag so UI/host can pick a player
//       result.reason = 'owner_choose_elimination';
//     } else {
//       result.reason = 'no_elimination';
//     }
